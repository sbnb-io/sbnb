...
CMDLINE="console=ttyS0 console=tty0 earlyprintk verbose amd_iommu=on intel_iommu=on iommu=pt pcie_acs_override=downstream,multifunction video=efifb:off video=vesa:off vfio_iommu_type1.allow_unsafe_interrupts=1 module_blacklist=nouveau,nvidiafb,snd_hda_intel dyndbg=\"module firmware_class +p; module microcode +p; module ccp +p\""
CMDLINE_TMP=$(mktemp)
OUTPUT=sbnb.efi
