...
class MQTTReconciler:
    ...
    def __init__(self):
        ...
        self._volume_caps = {}
        # True only in the data-plane process (reefy-reconciler). Control
        # handlers check this to decide: delegate to the data plane over
        # Varlink (control) vs. run the work locally (data plane).
        self._is_data_plane = False
    ...
    def _backup_now(self, payload, cmd_id=None):
        ...
        Control mode forwards to the data plane over Varlink."""
        if not self._is_data_plane:
    ...
    def _apply_desired_state(self, old_state=None):
        ...
        # Varlink (it owns mount/compose/restore, isolated). _apply_and_publish
        # still runs here in control and publishes the applying/ready stages
        # around this call. The data plane re-enters this method with
        # _is_data_plane=True and does the real work below.
        if not self._is_data_plane:
            try:
                with open(self.DESIRED_STATE_PATH) as f:
                    state_str = f.read()
        ...
        # Fair-share volume caps (path -> % of pool) consumed by
        # _ensure_volume_lv when it creates per-volume LVs.
        ...
        if app_volumes:
            self._prepare_app_dirs(app_volumes, backup_paths=backup_paths)
        ...
        if old_state is not None:
            self._reclaim_deleted_instance_lvs(old_state, state, backup_paths)
    ...
    # /mnt/reefy-data/state. Guarantees control-plane state always has
    # space even when the app pool hits 100% - so comms/identity survive
    # a full disk instead of going read-only with everything else.
    # Created only on fresh provision (the pool takes 100%FREE, so an
    # existing device has no room and keeps state on reefy_default).
    STATE_LV = 'reefy_state'
    # Legacy LV name used by pre-thin-pool installs — keep mounting it
    # for backward compat; those devices don't get the snapshot-backed
    # backup path until they factory-reset into the new layout.
    LEGACY_STORAGE_LV = 'data'
    REEFY_DATA_MNT = '/mnt/reefy-data'
    # ext4 mount opts used everywhere we mount a reefy LV (default LV,
    # per-volume backup LVs, USB-p4 fallback). Single source so all
    # mount sites stay in sync. `discard` is the TRIM-passthrough
    # half of the chain (the LUKS half is --allow-discards at open
    # time); without both, the drive's FTL doesn't reclaim freed
    # blocks. Kept identical for default + per-volume LVs since they
    # all live on the same drive with the same wear semantics.
    ...
    def _apply_storage(self, storage):
        ...
        self._ensure_lvm_stack(luks_pvs, _log=lambda m: log('reconciler', m))
        lv_path = self._active_reefy_lv_path()
        if not lv_path:
...
def main_data_plane():
    """Data plane (reefy-reconciler): serve Varlink, do the
    storage/container work, no MQTT."""
    try:
        MQTTReconciler().run_data_plane()
    except KeyboardInterrupt:
        sys.exit(0)
    except Exception as e:
...
def main_boot_mount():
    """Boot mount (reefy-mount-volumes): pre-docker oneshot that mounts
    per-app volumes from persisted desired-state, then exits, so docker
    (ordered after it) starts with volumes already in place."""
    try:
        MQTTReconciler().boot_mount()
    except Exception as e:
