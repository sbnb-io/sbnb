...
# This script automates Sbnb Linux bootable USB creation process on macOS.
...
diskutil list | grep "/dev/disk" | while read -r line; do
    deviceIdentifier=$(echo $line | awk '{print $1}')
    deviceName=$(echo $line | awk '{print $2, $3, $4, $5, $6, $7, $8, $9, $10}')
    diskSize=$(diskutil info $deviceIdentifier | grep "Disk Size" | awk '{print $3, $4}')
    deviceLocation=$(diskutil info $deviceIdentifier | grep "Device Location" | awk '{print $3}')
    diskIndex=$(echo $deviceIdentifier | grep -oE '[0-9]+')
    echo -e "${YELLOW}$diskIndex: $deviceIdentifier | $deviceName | $diskSize | $deviceLocation${NC}"
done

read -p "Enter the number of the disk to flash into (e.g., 2 for /dev/disk2): " selectedDiskNumber
selectedDrive="/dev/disk$selectedDiskNumber"
