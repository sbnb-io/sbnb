...
This document preserves the complete hardware investigation captured on
2026-06-29. The production implementation now uses the standard-UEFI
fresh-entry confirmation described below: it creates a new entry for each
slot, orders the active slot first, and removes the previous entries only
after the new pair is valid. Reefy does not write vendor-private EFI
variables.

For the concise current architecture and operational flow, see
[A/B firmware updates](./a-b-firmware-updates.md). The investigation record
below remains intentionally detailed so its board identities, variable
snapshots, failed experiments, and validation evidence are not lost.
